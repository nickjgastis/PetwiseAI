...
const GenerateReport = async (inputs, enabledFields) => {
  ...
  Doctor: Dr. ${inputs.doctor || "Provide here"}  
  ...
  ${getEnabledContent('physicalExamFindings', inputs.physicalExamFindings)
        ? `Correct and format the input below with proper medical terminology and systematic organization.
  Input: "${getEnabledContent('physicalExamFindings', inputs.physicalExamFindings)}"  
  Formatted Output:
  - Correct any misspelled medical terms
  - Ensure proper capitalization and measurements
  - Organize findings by body system
  - Use professional medical terminology
  - Include all abnormal and relevant normal findings`
...
[same format]`}` : ''}
  ...
  Formatted Output:`
        : `Generate naturopathic treatment options based on the diagnosis and treatment plan.`}` : ''}
  ...
  Formatted Output:`
        : `Generate a follow-up plan based on the treatment plan and diagnosis. State the next appointment date: [x days from today]. and the reason for the appointment. Each sentence should be on its own line.`}` : ''}
  ...
  Formatted Output:`
        : `Generate a LETTER that educates the client on their pets condition. After stating a diagosis or medical term, write the common term in parenthesis.Start with Dear [Client Name], and sign of with "/nThank you for the opertunity to help,/n [Doctor name]"`}` : ''}
